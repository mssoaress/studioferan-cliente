...
async function iniciar() {
  try {
    ...
    console.log("[Studio Ferran] OK —", B.servicosDB.length, "serviços");
  } catch (e) {
...
function renderServicos() {
  ...
  if (!B.servicosDB.length) {
    el.innerHTML =
      '<p style="color:var(--t3);padding:20px 0">Nenhum serviço encontrado.</p>';
    return;
  }
  el.innerHTML = B.servicosDB
    .map(function (s) {
      ...
      return (
        ...
        '<span class="serv-preco">R$ ' +
        fmtM(s.preco) +
        "</span>" +
        '<div class="serv-chk">' +
        '<svg viewBox="0 0 12 12" fill="none" width="10" height="10">' +
        '<path d="M2 6l3 3 5-5" stroke="' +
        (sel ? "var(--bg)" : "transparent") +
...
function toggleServ(id) {
  ...
  if (i > -1) B.servicos.splice(i, 1);
  else B.servicos.push(id);
  renderServicos();
}

function atualizarTotal() {
  var bar = document.getElementById("totalBar");
  if (!B.servicos.length) {
...
function renderSlots() {
  ...
  if (!B.data) {
    grid.innerHTML = '<p class="slot-empty">Selecione uma data primeiro.</p>';
    return;
  }
...
// ── NAVEGAÇÃO ─────────────────────────────────
function goStep(n) {
  if (n === 2 && !B.servicos.length) {
    toast("Selecione ao menos um serviço", true);
    return;
  }
  if (n === 3) {
    if (!B.data) {
      toast("Selecione uma data", true);
      return;
    }
    if (!B.horario) {
      toast("Selecione um horário", true);
      return;
    }
    renderResumo();
  }
...
function rcRow(k, v) {
  return (
    '<div class="rc-row"><span class="rc-key">' +
    k +
    '</span><span class="rc-val">' +
    v +
    "</span></div>"
  );
}
function rcRowTotal(k, v) {
  return (
    '<div class="rc-row total"><span class="rc-key">' +
    k +
    '</span><span class="rc-val">' +
    v +
    "</span></div>"
...
async function confirmar() {
  ...
  if (!tel) {
    toast("Informe seu WhatsApp", true);
    return;
  }
  ...
  try {
    ...
    ag.id = ar.id;
    B.agendamentosDB.push(ag);
    console.log("[Studio Ferran] Agendamento salvo:", ar.id);
  } catch (e) {
  ...
  document.getElementById("btnWpp").href =
    "https://wa.me/5583991630761?text=" + msg;
...
function sucRow(k, v) {
  return (
    '<div class="suc-row"><span>' + k + "</span><span>" + v + "</span></div>"
  );
}
...
function calcTotal() {
  return B.servicos.reduce(function (acc, id) {
    var s = B.servicosDB.find(function (x) {
      return x.id === id;
    });
...
function nomesServicos() {
  return B.servicos.map(function (id) {
    var s = B.servicosDB.find(function (x) {
      return x.id === id;
    });
    return s ? s.nome : id;
